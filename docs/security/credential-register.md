...
| `GITHUB_ISSUES_TOKEN` (development)                                   | Fine-grained token limited to the Sabiqah repository; Issues write and Metadata read only        | GitHub `development` environment; installed as a Cloudflare Worker secret by deployment  | Application maintainer | Review quarterly; rotate with overlap and verify one development selection report                      |
