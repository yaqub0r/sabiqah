...
| `GITHUB_ISSUES_TOKEN` (development Worker secret; `SABIQA_GITHUB_ISSUES_TOKEN` in GitHub) | Fine-grained token limited to the Sabiqah repository; Issues write and Metadata read only        | GitHub `development` environment; installed as a Cloudflare Worker secret by deployment  | Application maintainer | Review quarterly; rotate before 2027-02-09 with overlap and verify one development selection report    |
