...
| `GITHUB_ISSUES_TOKEN` (development)                                   | Fine-grained token limited to the Sabiqah repository; Issues write and Metadata read only        | Cloudflare Worker secret; recovery copy controlled by the account owner                  | Application maintainer | Review quarterly; rotate with overlap and verify one development selection report                      |
